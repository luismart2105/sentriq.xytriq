...
<body>
    ...
    <main class="document">
        <section class="page page-one">
            ...
            <div class="totals"><div><span>Subtotal de materiales</span><strong>${{ number_format($quote->materialsSubtotal(), 2) }}</strong></div><div><span>Instalación, configuración y puesta en marcha</span><strong>${{ number_format((float) $quote->installation_amount, 2) }}</strong></div><div class="grand-total"><span>Total del proyecto</span><strong>${{ number_format($quote->total(), 2) }} MXN</strong></div></div>
