    ...
    <main class="document">
        <section class="page page-one">
            ...
            <table class="items-table">
                ...
                <tbody>@foreach ($quote->items as $item)<tr><td><strong>{{ $item['concept'] }}</strong>@if($item['model'] ?? null)<small>{{ $item['model'] }}</small>@endif</td><td>{{ $item['benefit'] ?? '' }}</td><td>{{ number_format((float) $item['quantity'], (float) $item['quantity'] == floor((float) $item['quantity']) ? 0 : 2) }}</td><td>${{ number_format((float) $item['unit_price'], 2) }}</td><td>${{ number_format((float) $item['quantity'] * (float) $item['unit_price'], 2) }}</td></tr>@endforeach</tbody>
            </table>
