...
- El formulario está apagado por defecto hasta aprobar el texto legal. El CRM administrativo y la medición de clics no dependen de esta bandera.
...
SENTRIQ_LEAD_NOTIFICATION_EMAIL=correo-confirmado@example.com
```

Luisangel debe aprobar el aviso de privacidad provisional y confirmar el correo. La configuración observada en producción usa `MAIL_MAILER=log`, por lo que también debe configurarse un transporte de correo real antes de esperar avisos por email. Después de cambiar variables, ejecutar `php artisan config:clear`.

La primera versión del formulario está redactada para proyectos nuevos. Si también aceptará soporte/reparaciones, debe agregarse esa opción después de que Luisangel lo confirme.
...
5. Revisar el aviso de privacidad y configurar correo. Después cambiar `SENTRIQ_LEAD_FORM_ENABLED=true`, limpiar configuración y enviar un formulario de prueba.
